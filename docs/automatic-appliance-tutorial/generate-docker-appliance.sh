...
If the base image doesn't exist, it will be built automatically.
...
if [[ "$BASE_OS" == *".aarch64" ]]; then
    IS_ARM64="true"
    print_info "🔧 ARM64 build detected: $BASE_OS"

    # Warn about cross-architecture builds
    HOST_ARCH=$(uname -m)
    if [ "$HOST_ARCH" = "x86_64" ]; then
        print_warning "⚠️  Cross-architecture build: You are on x86_64 trying to build ARM64"
        print_warning "   Building ARM64 images on x86_64 requires QEMU software emulation,"
        print_warning "   which is extremely slow (hours) and may fail."
        print_warning "   Recommended: Build ARM64 images on ARM64 hardware."
    fi
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if [ ! -f "$BASE_IMAGE" ]; then
        print_info "📦 Base image not found: ${BASE_OS}.qcow2"
        print_info "  ℹ️  Building it now (this may take 10-15 minutes)..."
        ...
        print_info "🔨 Building base image: ${BASE_OS}..."
        ...
        print_info "  ✅ Base image found: ${BASE_OS}.qcow2"
    fi
