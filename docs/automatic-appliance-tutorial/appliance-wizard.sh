...
step_architecture() {
    ...
    echo -e "Select the target CPU architecture for your appliance.\n"
    print_info "Choose the architecture that matches your deployment environment."
    print_info "Most cloud and server environments use x86_64 (Intel/AMD)."
    echo ""
    ...
    local arch_options=("x86_64 (Intel/AMD) - Most common" "ARM64 (aarch64) - For ARM servers")
    ...
    if [ "$selected_idx" -eq 0 ]; then
        ARCH="x86_64"
        # Populate OS_LIST with x86 options
        OS_LIST=("${OS_LIST_X86[@]}")
    else
        ARCH="aarch64"
        # Populate OS_LIST with ARM options
        OS_LIST=("${OS_LIST_ARM[@]}")

        # Warn if trying to build ARM64 on x86_64 host
        if [ "$host_arch" = "x86_64" ]; then
            echo ""
            print_warning "Cross-architecture build detected!"
            echo ""
            echo -e "  ${YELLOW}You are on an x86_64 host trying to build ARM64 images.${NC}"
            echo -e "  ${DIM}Building ARM64 images on x86_64 requires QEMU software emulation,${NC}"
            echo -e "  ${DIM}which is extremely slow (hours instead of minutes) and may fail.${NC}"
            echo ""
            echo -e "  ${WHITE}Recommended:${NC} Build ARM64 images on ARM64 hardware."
            echo ""
            prompt_yes_no "Continue with ARM64 anyway?" CONTINUE_ARM "false"
            if [ "$CONTINUE_ARM" != "true" ]; then
                # Go back and let user choose x86_64
                return $NAV_BACK
            fi
        fi
    fi
...
step_base_os() {
    ...
    echo -e "Select the base operating system for your ${WHITE}${arch_display}${NC} appliance.\n"
...
step_summary() {
    clear_screen
    print_header
    print_step 8 $TOTAL_STEPS "Summary"
