...
SRC="/opt/one-appliance/open-ondemand-src"           ### Where this script unpacks the source
APPLIANCE_DIR="/opt/open-ondemand"                   ### Where the appliance code lives in the image
...
/etc/open-ondemand/build.env records what the image was built from.
DESC
)
...
service_install()
{
    _open_ondemand_write_source || { msg error "could not unpack the appliance source"; return 1; }
...
[Open OnDemand]
portal      = $(cat /etc/open-ondemand/portal-url 2>/dev/null || printf 'https://%s/\n' "${ONEAPP_PORTAL_HOST_NAME:-$(hostname -I | awk '{print $1}')}")
...
built_from  = /etc/open-ondemand/build.env
...
# and the appliance code in /opt/open-ondemand. What is deleted is written again by the
...
umount -l /var/lib/open-ondemand/slurm 2>/dev/null || true
sed -i '\#:/export/home /home nfs4 #d;\# /cvmfs/software.eessi.io cvmfs #d;\#:/export/software /opt/eessi nfs4 #d;\# /var/lib/open-ondemand/slurm nfs4 #d' /etc/fstab
...
sed -i '/# open-ondemand worker$/d;/# open-ondemand portal$/d' /etc/hosts
...
rm -f /etc/munge/munge.key /etc/slurm/slurm.conf /etc/slurm/cgroup.conf /etc/slurm/gres.conf \
      /etc/slurm/slurmdbd.conf /etc/open-ondemand/slurmdbd.pass /etc/open-ondemand/ldap-admin.pass \
      /etc/open-ondemand/nfs.env /etc/open-ondemand/nfs_host /etc/open-ondemand/portal-url
...
rm -rf /var/lib/open-ondemand/slurm /var/spool/slurmctld /var/spool/slurmd /var/lib/ood-slurm \
...
rm -rf /etc/letsencrypt /etc/ssl/open-ondemand 2>/dev/null || true
ok "LDAP tree, Open OnDemand site configuration and certificates removed"

# Storage role, the exports and the proxy cache.
rm -f /etc/exports.d/open-ondemand*.exports
rm -f /etc/squid/conf.d/open-ondemand-cvmfs.conf
...
echo "open-ondemand" > /etc/hostname
hostnamectl set-hostname open-ondemand 2>/dev/null || true
ok "machine-id, name, host keys and leases deleted"

msg "reducing the size of the image"
apt-get clean >/dev/null 2>&1 || true
rm -rf /var/lib/apt/lists/* /var/tmp/* /tmp/* /root/.cache /root/open-ondemand 2>/dev/null || true
...
for req in /etc/open-ondemand/build.env /etc/open-ondemand/ood-app-lib.sh \
           /etc/open-ondemand/onegate-lib.sh /etc/open-ondemand/code-server.env \
           /opt/ood/linuxhost.sif /opt/open-ondemand/worker/configure.sh \
           /opt/open-ondemand/scripts/30-configure-portal.sh \
           /opt/open-ondemand/storage/10-install-nfs.sh \
           /opt/open-ondemand/scripts/40-configure-slurm-controller.sh \
           /opt/open-ondemand/config/slurm/slurm.conf.tpl \
...
for secret in /etc/open-ondemand/ldap-admin.pass /etc/open-ondemand/slurmdbd.pass /etc/munge/munge.key; do
...
#!/usr/bin/env bash
# Boot phase of the open-ondemand appliance, it picks the role and configures it.
#
...
DIR="${ONEAPP_APPLIANCE_DIR:-/opt/open-ondemand}"
ROLE="${ONEAPP_ROLE:-}"
...
report_failure() {
    ...
    if . /etc/open-ondemand/onegate-lib.sh 2>/dev/null && onegate_ready; then
        onegate_call vm update --data "ERROR=\"open-ondemand ${ROLE:-?}: ${msg:-configuration failed, see ${LOG}}\"" \
            >/dev/null 2>&1 || true
    fi
}
...
if . /etc/open-ondemand/onegate-lib.sh 2>/dev/null && onegate_ready; then
    ...
    if [[ "$ROLE" == "portal" && -s /etc/open-ondemand/portal-url ]]; then
        onegate_call vm update --data "OOD_URL=$(cat /etc/open-ondemand/portal-url)" >/dev/null 2>&1 \
            && ok "OOD_URL published: $(cat /etc/open-ondemand/portal-url)" \
...
#!/usr/bin/env bash
# Build phase of the open-ondemand appliance, one image for the three roles.
#
...
DEST="${ONEAPP_APPLIANCE_DIR:-/opt/open-ondemand}"
t0=$(date +%s)
...
# --- provenance ------------------------------------------------------------------------------
cat >> /etc/open-ondemand/build.env <<EOF
APPLIANCE=open-ondemand
APPLIANCE_ROLES=portal,storage,worker
APPLIANCE_DIR=${DEST}
OOD_VERSION=$(dpkg-query -W -f='${Version}' ondemand 2>/dev/null)
EOF
ok "provenance extended in /etc/open-ondemand/build.env"
...
# in /opt/open-ondemand/bin/slurm, the pattern the AWS and Azure integrations use. The
...
v2:
  ...
  job:
    ...
    bin: "/usr/bin"
    bin_overrides:
      sbatch: "/opt/open-ondemand/bin/slurm/sbatch"
      squeue: "/opt/open-ondemand/bin/slurm/squeue"
      scancel: "/opt/open-ondemand/bin/slurm/scancel"
      scontrol: "/opt/open-ondemand/bin/slurm/scontrol"
      sinfo: "/opt/open-ondemand/bin/slurm/sinfo"
      sacct: "/opt/open-ondemand/bin/slurm/sacct"
      sacctmgr: "/opt/open-ondemand/bin/slurm/sacctmgr"
...
TaskProlog=/opt/open-ondemand/worker/slurm-task-prolog.sh
TaskEpilog=/opt/open-ondemand/worker/slurm-task-epilog.sh
...
# /etc/open-ondemand/ldap-admin.pass. ldap_admin_pass below resolves it.
...
backup_once() {
    local f="$1"
    [[ -f "$f" && ! -f "${f}.open-ondemand.orig" ]] && cp -a "$f" "${f}.open-ondemand.orig"
    return 0
}
...
ldap_admin_pass() {
    local f=/etc/open-ondemand/ldap-admin.pass
    if [[ -n "$ONEAPP_LDAP_ADMIN_PASS" ]]; then
        printf '%s' "$ONEAPP_LDAP_ADMIN_PASS"
    elif [[ -s "$f" ]]; then
        cat "$f"
    else
        install -d -m 755 /etc/open-ondemand
        local pw
...
# Generated by open-ondemand/scripts/20-install-identity.sh
[sssd]
...
probe="/home/.open-ondemand-probe.$$"
...
install -d -m 755 /etc/open-ondemand
printf 'https://%s/\n' "$SERVERNAME" > /etc/open-ondemand/portal-url
...
# The certificate of the switches is installed on every run, so a switch turned on later on
# a running portal replaces the self-signed one. Only the self-signed mode keeps an existing
# certificate, whatever mode issued it.
...
if [[ ! -L "$cert" ]]; then
    trust=/usr/local/share/ca-certificates/open-ondemand-portal.crt
...
cat > "$PORTAL_YML" <<EOF
# Generated by open-ondemand/scripts/30-configure-portal.sh
...
pun_pre_hook_root_cmd: '/opt/open-ondemand/bin/pun_prehook'
...
# more setup.
mkdir -p /opt/open-ondemand/bin
cat > /opt/open-ondemand/bin/pun_prehook <<'HOOK'
#!/usr/bin/env bash
...
if [[ ! -f "$key" ]]; then
    ...
    if runuser -u "$user" -- ssh-keygen -q -t ed25519 -N "" -C "open-ondemand-portal" -f "$key" </dev/null; then
...
if [[ -x /opt/open-ondemand/bin/pun_prehook_site ]]; then
    /opt/open-ondemand/bin/pun_prehook_site "$user" \
...
sed -i "s|@@PORTAL_COMPUTE_IP@@|${PORTAL_COMPUTE_IP}|g" /opt/open-ondemand/bin/pun_prehook
if grep -q '@@PORTAL_COMPUTE_IP@@' /opt/open-ondemand/bin/pun_prehook; then
    die "the portal address was not substituted in the pre-PUN hook"
fi
chmod 755 /opt/open-ondemand/bin/pun_prehook
ok "pre-PUN hook installed at /opt/open-ondemand/bin/pun_prehook"
...
cat > /etc/ood/config/ondemand.d/open-ondemand.yml <<'EOF'
# Generated by open-ondemand/scripts/30-configure-portal.sh
bc_dynamic_js: true
nav_bar:
  - "Files"
    ...
    - columns:
        - width: 12
          widgets:
            ...
            - motd
EOF
ok "menu and dynamic forms declared in /etc/ood/config/ondemand.d/open-ondemand.yml"
...
install -d -m 755 /etc/open-ondemand
install -m 644 "${REPO_ROOT}/worker/onegate-lib.sh" /etc/open-ondemand/onegate-lib.sh
bash -n /etc/open-ondemand/onegate-lib.sh || die "onegate-lib.sh is not valid bash"
...
STATE_MOUNT=/var/lib/open-ondemand/slurm
DEF_MEM_PER_CPU="${ONEAPP_SLURM_DEF_MEM_PER_CPU:-1024}"
PASS_FILE=/etc/open-ondemand/slurmdbd.pass
CLUSTER=ood
...
install -d -m 755 /etc/open-ondemand "$STATE_MOUNT"
if [[ -n "$NFS_HOST" ]]; then
...
cat > /etc/systemd/system/slurmctld.service.d/open-ondemand.conf <<UNIT
# Generated by open-ondemand/scripts/40-configure-slurm-controller.sh
[Unit]
...
{
    printf '# Generated by open-ondemand/scripts/70-install-cvmfs.sh\n'
...
#
# Usage:  ./71-eessi-warmup.sh   (reads /etc/open-ondemand/eessi.env)
...
if out="$(bash -c "source '${init}' >/dev/null 2>&1 && module load '${EESSI_JUPYTER_MODULE}' && python -c 'import ipykernel'" 2>&1)"; then
    ...
    printf '%s\n' "$out" | tail -n 8 | sed 's/^/    /'
    if . /etc/open-ondemand/onegate-lib.sh 2>/dev/null && onegate_ready; then
        onegate_call vm update --data "ERROR=\"open-ondemand portal: could not load ${EESSI_JUPYTER_MODULE} from EESSI ${EESSI_VERSION}\"" \
            >/dev/null 2>&1 || true
    fi
...
PROXY_DIR=/opt/open-ondemand/bin/slurm
STATE_DIR=/etc/open-ondemand
...
menu=/etc/ood/config/ondemand.d/open-ondemand.yml
...
# Generated by open-ondemand/scripts/cvmfs-client.sh
...
REFRESH_SECONDS = 30
ONEGATE = ["bash", "-c",
           ". /etc/open-ondemand/onegate-lib.sh && onegate_ready && "
...
#
# Usage:  ood-slurm-backup /var/lib/open-ondemand/slurm/backup
set -u -o pipefail
...
ONEGATE_LIB="${ONEGATE_LIB:-/etc/open-ondemand/onegate-lib.sh}"
...
set -u
cmd="$(basename "$0")"
controller="$(cat /etc/open-ondemand/slurm_controller 2>/dev/null || true)"
...
EXPORTS_FILE=/etc/exports.d/open-ondemand.exports
STATE_DIR=/etc/open-ondemand
...
msg "writing /etc/squid/conf.d/open-ondemand-cvmfs.conf"
backup_once /etc/squid/squid.conf
{
    printf '# Generated by open-ondemand/storage/20-install-squid.sh\n'
...
EOF
} > /etc/squid/conf.d/open-ondemand-cvmfs.conf
...
#
# Reads /etc/open-ondemand/nfs.env, written by storage/10-install-nfs.sh.
set -u

ENV_FILE=/etc/open-ondemand/nfs.env
EXPORTS_FILE=/etc/exports.d/open-ondemand.exports
ONEGATE_LIB="${ONEGATE_LIB:-/etc/open-ondemand/onegate-lib.sh}"
...
content="$({
    printf '# Generated by open-ondemand/storage/export-refresh.sh, rewritten when the portal changes\n'
    for ip in $admin; do
...
sed -i '/# open-ondemand worker$/d' /etc/hosts
...
for req in /etc/open-ondemand/build.env /etc/open-ondemand/ood-app-lib.sh "$SIF_PATH"; do
...
ok "worker image: $(sed -n 's/^BUILD_DATE=//p' /etc/open-ondemand/build.env)"
...
sed -i '/# open-ondemand worker$/d' /etc/hosts
printf '%s %s %s # open-ondemand worker\n' "$priv_ip" "$fqdn" "$name" >> /etc/hosts
...
if [[ -n "$LDAP_HOST" ]]; then
    ...
    backup_once /etc/sssd/sssd.conf
    cat > /etc/sssd/sssd.conf <<EOF
# Generated by open-ondemand/worker/configure.sh
[sssd]
config_file_version = 2
services = nss, pam
domains = ood
...
if [[ -n "$LDAP_HOST" ]]; then
    ...
    sed -i '/# open-ondemand portal$/d' /etc/hosts
    printf '%s ood-portal # open-ondemand portal\n' "$LDAP_HOST" >> /etc/hosts
    # shellcheck source=/dev/null
    . /etc/open-ondemand/onegate-lib.sh
...
')"
    ...
    printf "# Generated by open-ondemand/worker/configure.sh\nSLURMD_OPTIONS='--conf-server ood-portal:%s -Z --conf \"%s\"'\n" \
...
printf '# Generated by open-ondemand/worker/install.sh\nCODE_SERVER_VERSION=%s\nCODE_SERVER_BIN=%s\n' \
    "$CODE_SERVER_VERSION" "$CODE_SERVER_BIN" > /etc/open-ondemand/code-server.env
chmod 644 /etc/open-ondemand/code-server.env
...
# Generated by open-ondemand/worker/install.sh
...
install -m 644 "${HERE}/ood-app-lib.sh" /etc/open-ondemand/ood-app-lib.sh
bash -n /etc/open-ondemand/ood-app-lib.sh || die "ood-app-lib.sh is not valid bash"
ok "/etc/open-ondemand/ood-app-lib.sh installed"
...
cat > /etc/systemd/system/slurmd.service.d/open-ondemand.conf <<'UNIT'
[Unit]
...
RestartSec=20
UNIT
install -m 644 "${HERE}/onegate-lib.sh" /etc/open-ondemand/onegate-lib.sh
bash -n /etc/open-ondemand/onegate-lib.sh || die "onegate-lib.sh is not valid bash"
...
install -d -m 755 /opt/open-ondemand/worker /opt/open-ondemand/scripts
install -m 755 "${HERE}/configure.sh"          /opt/open-ondemand/worker/configure.sh
install -m 644 "${HERE}/../scripts/00-lib.sh"  /opt/open-ondemand/scripts/00-lib.sh
install -m 755 "${HERE}/../scripts/cvmfs-client.sh" /opt/open-ondemand/scripts/cvmfs-client.sh
...
/opt/open-ondemand/worker/configure.sh || { echo "the boot phase failed"; exit 1; }
...
SIF_PATH=${SIF_PATH}
EOF
chmod 644 /etc/open-ondemand/build.env
ok "provenance in /etc/open-ondemand/build.env"
...
#
# Usage:  source /etc/open-ondemand/onegate-lib.sh
...
#
# It lives on the worker VM, at /etc/open-ondemand/ood-app-lib.sh, and worker/install.sh
...
ood_modules() {
    ...
    for m in "$@"; do
        if [[ -z "${m}" ]]; then
            ...
            echo "check EESSI_* in /etc/open-ondemand/eessi.env on the portal" >&2
            return 1
        fi
...
#
# /etc/open-ondemand/onegate-lib.sh resolves the OneGate endpoint and explains why the
...
DRAIN_REASON="open-ondemand scale-down"
...
follow_portal() {
    local current new
    current="$(awk '/# open-ondemand portal$/ {print $1}' /etc/hosts | head -1)"
    new="$(portal_addr)"
    [[ -n "$new" && -n "$current" && "$new" != "$current" ]] || return 0
    log "the portal moved from ${current} to ${new}, following it"
    sed -i "s/^${current} ood-portal # open-ondemand portal$/${new} ood-portal # open-ondemand portal/" /etc/hosts
...
# it and records its path in /etc/open-ondemand/code-server.env. code-server serves
...
env_file=/etc/open-ondemand/code-server.env
if [[ ! -r "${env_file}" ]]; then
...
<%-
  eessi = {}
  eessi_env = '/etc/open-ondemand/eessi.env'
...
set -o pipefail
source /etc/open-ondemand/ood-app-lib.sh
