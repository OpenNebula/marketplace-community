...
# Redis Cache Appliance Installation Script
...
cat > /usr/local/bin/start-redis-container.sh << 'CONTAINER_SCRIPT'
#!/bin/bash
...
CONTAINER_PORTS="${CONTAINER_PORTS:-6379:6379}"
...
chmod +x /usr/local/bin/start-redis-container.sh

# Create systemd service for the container
cat > /etc/systemd/system/redis-container.service << 'SERVICE_EOF'
[Unit]
Description=Redis Cache Container Service
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/bin/start-redis-container.sh
ExecStop=/usr/bin/docker stop redis-cache
ExecStopPost=/usr/bin/docker rm redis-cache
TimeoutStartSec=300
Restart=on-failure
...
cat > /etc/profile.d/99-redis-welcome.sh << 'WELCOME_EOF'
#!/bin/bash
case $- in
    *i*) ;;
      *) return;;
...
echo "=================================================="
echo "  Redis Cache Appliance"
echo "=================================================="
echo "  Docker Image: redis:alpine"
echo "  Container: redis-cache"
echo "  Ports: 6379:6379"
echo ""
echo "  Commands:"
echo "    docker ps                    - Show running containers"
echo "    docker logs redis-cache   - View container logs"
echo "    docker exec -it redis-cache /bin/bash - Access container"
echo ""
